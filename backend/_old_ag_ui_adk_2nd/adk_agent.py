...
class ADKAgent:
    ...
    def __init__(
        self,
        # ADK Agent instance (optional if app is provided)
        adk_agent: Optional[BaseAgent] = None,
        
        # App object (optional if adk_agent is provided)
        app: Optional[App] = None,
        
        # App identification
        app_name: Optional[str] = None,
        
        # Remote Agent Client (optional)
        ...
        Args:
            ...
            remote_client: Client object for remote agent execution
        ...
        if not app and not adk_agent and not remote_client:
            raise ValueError("Must specify either 'app', 'adk_agent', or 'remote_client'")
        ...
        self._app = app
        self._adk_agent = adk_agent or (app.root_agent if app else None)
        ...
        # Initialize RemoteAgentWrapper if client provided
    ...
    async def rewind_session(self, session_id: str, invocation_id: str) -> None:
        ...
        if not (app_name and user_id):
             raise ValueError(f"Could not resolve app_name and user_id for session {session_id}")
    ...
    async def _start_new_execution(
        ...
        *,
        ...
        try:
            ...
            invocation_id = input.forwarded_props.get("invocation_id") if input.forwarded_props else None
    ...
    async def _start_background_execution(
        ...
        *,
        ...
        if invocation_id is not None:
            run_kwargs["invocation_id"] = invocation_id
    ...
    async def _run_adk_in_background(
        ...
        try:
            ...
            # Create runner or use remote wrapper
            if self._remote_agent_wrapper:
                runner = self._remote_agent_wrapper
            else:
                runner = self._create_runner(
                    adk_agent=adk_agent,
                    user_id=user_id,
