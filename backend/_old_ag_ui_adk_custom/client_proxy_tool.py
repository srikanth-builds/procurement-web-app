...
class ClientProxyTool(BaseTool):
    ...
    def __init__(
        ...
        Args:
            ...
            tool_config: Optional configuration for the tool (e.g. summarize_response)
        """
    ...
    async def _execute_proxy_tool(self, args: Dict[str, Any], tool_context: Any) -> Any:
        ...
        try:
            ...
            # Determine if we should summarize the response for the LLM
            # Default to False for UI tools to avoid verbosity
            ...
            # Return a response to satisfy the LLM protocol (avoids 400 Bad Request)
            response_content = {"status": "ok", "tool_name": self.ag_ui_tool.name}
            
            if should_summarize:
                # Provide more detail if the LLM is going to see it
                response_content["message"] = f"Tool '{self.ag_ui_tool.name}' executed successfully."
                logger.debug(f"Returning detailed response for proxy tool {self.ag_ui_tool.name}")
            else:
                logger.debug(f"Returning minimal response for proxy tool {self.ag_ui_tool.name}")
