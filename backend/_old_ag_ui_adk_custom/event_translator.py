...
class EventTranslator:
    ...
    def __init__(
        ...
        self._last_streamed_text: Optional[str] = (
            None  # Snapshot of most recently streamed text
        )
        self._last_streamed_run_id: Optional[str] = (
            None  # Run identifier for the last streamed text
        )
        self._last_streamed_text: Optional[str] = (
            None  # Snapshot of most recently streamed text
        )
    ...
    async def translate(
        ...
        """
        try:
            logger.info("Translating ADK event to AG-UI protocol events")
            logger.info("=" * 60)
            logger.debug(f"📥 ADK Event: {adk_event}")
            logger.info("=" * 60)
            ...
            # CRITICAL FIX: Update local session state immediately if there's a delta
            # This ensures that subsequent processing (like get_activity) sees the new state.
            if (
            ...
            # Handle ADK errors (e.g. MALFORMED_FUNCTION_CALL)
            if getattr(adk_event, "error_code", None) or getattr(
                adk_event, "error_message", None
            ):
                error_code = getattr(adk_event, "error_code", "ADK_ERROR")
                error_msg = getattr(adk_event, "error_message", "Unknown ADK error")
                logger.error(f"Translating ADK error: {error_code} - {error_msg}")

                from ag_ui.core import RunErrorEvent

                yield RunErrorEvent(
                    type=EventType.RUN_ERROR,
                    message=f"ADK Error: {error_msg}",
                    code=str(error_code),
                )
            ...
            if hasattr(adk_event, "get_function_calls"):
                ...
                if function_calls:
                    ...
                    if non_lro_calls:
                        logger.debug(
                            f"ADK function calls detected (non-LRO): {len(non_lro_calls)} of {len(function_calls)} total"
                        )
            ...
            # Check for both custom_metadata (standard ADK) and custom_data (legacy/alternative)
            custom_data = getattr(adk_event, "custom_metadata", None) or getattr(
                adk_event, "custom_data", None
            )

            if custom_data:
                yield CustomEvent(
                    type=EventType.CUSTOM, name="adk_metadata", value=custom_data
                )
    ...
    async def _translate_text_content(
        ...
        for part in adk_event.content.parts:
            ...
            if is_thought:
                # Deduplication: If this is the final response and we've already streamed thoughts for this run,
                # skip re-emitting them to avoid duplicates at the end.
                ...
                # If we were reasoning, we need to close it
                ...
                if part.text:  # Note: part.text == "" is False
                    # Filter out "For context:" parts generated by ADK for tool calls
                    if "For context:" in part.text:
                        logger.debug(
                            f"Skipping 'For context' text part: {part.text[:50]}..."
                        )
                        continue
        ...
        logger.info(
            f"📥 Text event - partial={is_partial}, turn_complete={turn_complete}, "
        ...
        if is_final_response:
            ...
            if self._is_streaming and self._streaming_message_id:
                ...
                logger.info(
                    f"📤 TEXT_MESSAGE_END (from final response): {end_event.model_dump_json()}"
                )
                yield end_event
            ...
            is_duplicate = (
                ...
                and combined_text == self._last_streamed_text
            )
            ...
            if is_duplicate:
                ...
                if combined_text:
                    ...
                    logger.debug("Skipping empty final response text event")
            ...
            for i, part in enumerate(adk_event.content.parts):
                if part.function_call:
                    if not long_running_function_call and part.function_call.id in (
                        ...
                        if (
                            hasattr(long_running_function_call, "args")
                            and long_running_function_call.args
                        ):
                            # Convert args to string (JSON format)
                            import json

                            args_str = (
                                json.dumps(long_running_function_call.args)
                                if isinstance(long_running_function_call.args, dict)
                                else str(long_running_function_call.args)
                            )
    ...
    async def _translate_function_calls(
        ...
        for func_call in function_calls:
            ...
            logger.info(f"Activity Registry: {self.activity_registry}")
            # --- SMART ACTIVITY STATUS (Registry Based) ---
            if self.activity_registry:
                ...
                logger.debug(
                    f"Checking activity for tool: {func_call.name} with args: {tool_args}"
                )
                ...
                logger.debug(f"Activity text for {func_call.name}: {activity_text}")
                ...
                logger.debug("No activity registry configured")
            # -----------------------------
            ...
            if hasattr(func_call, "args") and func_call.args:
                ...
                # Update the active tool call with args for later use in completion
                if tool_call_id in self._active_tool_calls:
                    self._active_tool_calls[tool_call_id] = {
                        "id": tool_call_id,
                        "name": func_call.name,
                        "args": (
                            func_call.args if isinstance(func_call.args, dict) else {}
                        ),
                    }
            ...
            # Clean up tracking - MOVED to _translate_function_response
    ...
    async def _translate_function_response(
        ...
        for func_response in function_response:
            ...
            # --- SMART ACTIVITY STATUS (Completed/Failed) ---
            if self.activity_registry:
                # We need the tool name. FunctionResponse has 'name'.
                tool_name = getattr(func_response, "name", None)
                if tool_name:
                    # Retrieve original args from active tool calls
                    tool_args = {}
                    if tool_call_id in self._active_tool_calls and isinstance(
                        self._active_tool_calls[tool_call_id], dict
                    ):
                        tool_args = self._active_tool_calls[tool_call_id].get(
                            "args", {}
                        )
                    ...
                    ) and response_content.startswith("Error:"):
                    ...
                    if error_message:
                        activity_text = self.activity_registry.get_activity(
                            ...
                            error=error_message,
                            session_state=self.session_state,
                            tool_args=tool_args,  # Pass original args
                        )
                        activity_type = "tool_execution_failed"
                    else:
                        # Coerce response to ensure it's a dict/primitive for the registry
                        # CRITICAL FIX: Handle MCP-style responses where the actual data is a JSON string
                        # inside a 'content' list: {'content': [{'type': 'text', 'text': 'JSON_HERE'}]}
                        response_payload = func_response.response

                        # 1. Handle direct JSON string
                        ...
                        # 2. Handle MCP structured response (dict with content list)
                        elif (
                            isinstance(response_payload, dict)
                            and "content" in response_payload
                        ):
                            try:
                                content_list = response_payload["content"]
                                        ...
                                        if (
                                            ...
                                            try:
                                                ...
                                                if isinstance(parsed_inner, dict):
                                                    # Merge the parsed inner fields into the main payload
                                                    # This allows {name} to be found even if it was inside the text block
                                                    response_payload = (
                                                    ...
                                                    break  # Assume the first valid JSON block is the main result
                                            except (json.JSONDecodeError, TypeError):
                                                pass
                            except Exception as e:
                                logger.warning(
                                    f"Failed to parse inner MCP content: {e}"
                                )

                        coerced_response = _coerce_tool_response(response_payload)
                        ...
                        activity_type = "tool_execution_completed"
                    ...
                    if activity_text:
                        yield ActivitySnapshotEvent(
                            ...
                            content={
                                ...
                                "status": (
                                    "failure"
                                    if activity_type == "tool_execution_failed"
                                    else "completed"
                                ),
                                "tool": tool_name,
                                "tool_id": tool_call_id,
                ...
                # Clean up tracking now that we're done with the tool call
                self._active_tool_calls.pop(tool_call_id, None)
            # -----------------------------------------
    ...
    def _create_state_delta_event(
        self, state_delta: Dict[str, Any], thread_id: str, run_id: str
    ) -> StateDeltaEvent:
    ...
    def _create_state_snapshot_event(
        ...
        FullSnapShot = {
            "context": {
                ...
                "app": {"version": state_snapshot.get("app_version", "unknown")},
            },
            "state": state_snapshot,
        }
        logger.debug(
            f"Creating StateSnapshotEvent with state: {json.dumps(state_snapshot, default=str)}"
        )

        return StateSnapshotEvent(type=EventType.STATE_SNAPSHOT, snapshot=FullSnapShot)
    ...
    async def force_close_streaming_message(self) -> AsyncGenerator[BaseEvent, None]:
        ...
        # Also force close reasoning if active
