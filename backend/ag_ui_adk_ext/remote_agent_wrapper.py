# ag_ui_adk_ext/remote_agent_wrapper.py

"""Wrapper for remote agent clients to provide a Runner-like interface.

This module provides RemoteAgentWrapper that adapts a remote agent client
(with async_stream_query or stream_query methods) to work with the ADKAgent
execution flow.
"""
...
class RemoteAgentWrapper:
    ...
    This allows using remote/deployed agents through the same interface
    as local ADK agents.

    Example:
        ```python
        from some_remote_client import RemoteAgentClient

        client = RemoteAgentClient(endpoint="https://agent.example.com")
        agent = ExtendedADKAgent(remote_client=client)
        ```
    """
    ...
    def __init__(self, client: Any):
        ...
        Args:
            client: An object with an `async_stream_query` or `stream_query` method.
        """
        self._client = client
    ...
    async def run_async(
        ...
        *,
        ...
        if new_message:
            # Convert Content to dict as expected by client
        ...
        # Pass invocation_id and state_delta if provided
        if invocation_id:
        ...
        if hasattr(self._client, "async_stream_query"):
            ...
            logger.warning(
                "Client does not have async_stream_query, using stream_query (might block)"
            )
            iterator = self._client.stream_query(**kwargs)
        else:
            raise AttributeError(
                "Client must have async_stream_query or stream_query method"
            )
        ...
        async for chunk in iterator:
            ...
            if isinstance(chunk, dict):
                ...
                logger.warning(
                    f"Received unknown chunk type from remote: {type(chunk)}"
                )
